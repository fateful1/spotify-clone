...
export default function MainPage({
    ...
    //если я не сделаю флаги загрузки компонентов, то страница крашится, говоря, что компонент пустой, то есть {} или [].
    //как можно, грубо говоря, подождать пока переменные заполнятся через запрос?
    let profileFlag = false;
    if (Object.keys(profile).length !== 0 && !profile.error) {
        profileFlag = true;
    }
    let playlistsFlag = false;
    if (Object.keys(playlists).length !== 0 && !playlists.error) {
        playlistsFlag = true;
    }
    let recommendFlag = false;
    if (Object.keys(recommend).length !== 0 && !recommend.error) {
        recommendFlag = true;
    }
            ...
            <div className={styles.root__mainmenu}>
                <div>
                    <h2 className={styles.mainmenu__title}>
                        Не пропусти топовые плейлисты
                    </h2>
                    <div className={styles.mainmenu__playlists}>
                        {playlistsFlag ? playlists.items.map((item) => {
                            return <Playlist 
                                key={item.id}
                                image={item.images[0]}
                                name={item.name}
                        ...
                        {recommendFlag ? recommend.playlists.items.map((item) => {
                            return <Playlist 
                                key={item.id}
                                image={item.images[0]}
                                name={item.name}
